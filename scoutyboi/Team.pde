...
public class Team {
  ...
  private ArrayList<Float> totalPofloats;
  ...
  public Team(Integer teamNum, Table dataTable) {
    ...
    totalPofloats = new ArrayList<Float>();
    setData();
  }
  ...
  public void setData() {
    for (TableRow row : dataTable.rows()) {
      ...
      if (tempTeamNum == getTeamNum()) {
        ...
        totalPofloats.add(totalCargo.get(totalCargo.size() - 1) * 3 + totalHatch.get(totalHatch.size() - 1) * 2);
      }
    }
  }
  ...
  public Float averageTotalPofloats() {
    return util.averageValue(totalPofloats);
  }
