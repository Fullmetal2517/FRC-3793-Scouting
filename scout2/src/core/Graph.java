...
public class Graph {
	...
	public Graph(ArrayList<Team> t, Method method) {// reflection only works with a method that returns an int
		...
		scale = (Utility.WINDOW_HEIGHT - 200) / Utility.largestValue(printingValues);
	...
	public void setPrintingValues() {
		...
		for (int i = 0; i < teams.size() - 1; i++) {
			 try {
			printingValues.add((Integer) method.invoke(teams.get(i), method));
			 } catch (Exception e) {
			 e.printStackTrace();
			 }
			 System.out.println(printingValues.size() + " " + printingValues.get(0));
		}
	}
}
